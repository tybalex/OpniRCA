...
def span_encoding(input_data, cluster_id: str, is_history=False) -> pd.DataFrame:
    ...
    if is_history:
        encoded_history_span = read_pkl_file(cluster_id + "encoded_history_span.pkl")
    ...
    for trace in input_data:
        indices = np.asarray(
            [
                idx
                for idx, (source, target) in enumerate(trace["s_t"])
                if source != target
            ]
        )
        if len(indices) <= 0:
        ...
        if ENABLE_ALL_FEATURES:
            ...
            data["trace_start_timestamp"].extend(
                min(trace["timestamp"]) / 1e6 for _ in trace["timestamp"]
            )
            data["trace_end_timestamp"].extend(
                max(trace["endtime"]) / 1e6 for _ in trace["endtime"]
            )
            ...
            data["mem_use_amount"].extend(
                _ / 1e12 for _ in trace["mem_use_amount"]
            )  # 1000MB disabled
            data["file_write_rate"].extend(
                _ / 1e12 for _ in trace["file_write_rate"]
            )  # 100MB
            data["file_read_rate"].extend(
                _ / 1e12 for _ in trace["file_read_rate"]
            )  # 100MB
            ...
            data["http_status"].extend(
                int(_) // 100 if _ != 0 else 9 for _ in trace["http_status"]
            )
        else:
            data["start_timestamp"].extend(_ for _ in trace["timestamp"])
            data["end_timestamp"].extend(_ for _ in trace["endtime"])
            data["trace_start_timestamp"].extend(
                min(trace["timestamp"]) for _ in trace["timestamp"]
            )
            data["trace_end_timestamp"].extend(
                max(trace["endtime"]) for _ in trace["endtime"]
            )
            data["trace_label"].extend(trace["label"] for _ in trace["s_t"])
            data["trace_id"].extend(trace["trace_id"] for _ in trace["s_t"])
    ...
    if is_history:
        write_pkl_file(cluster_id + "encoded_history_span.pkl", df)
    return df
...
def encoding_data(source_data: List, drop_service=(), drop_fault_type=()):
    ...
    if ENABLE_ALL_FEATURES:
        _data = (
            np.ones((len(source_data), len(INVOLVED_SERVICES), 9), dtype=np.float32)
            * -1
        )
    else:
        _data = (
            np.ones((len(source_data), len(INVOLVED_SERVICES), 2), dtype=np.float32)
            * -1
        )
    ...
    for trace_idx, trace in enumerate(source_data):
        if "fault_type" in trace and trace["fault_type"] in drop_fault_type:
            continue
        if "root_cause" in trace and any(
            _ in drop_service for _ in trace["root_cause"]
        ):
            continue
        indices = np.asarray(
            [
        ...
        if ENABLE_ALL_FEATURES:
            ...
            _data[trace_idx, service_idx, 2] = np.asarray(
                [round(_, 2) for _ in trace["mem_use_percent"]]
            )
            _data[trace_idx, service_idx, 3] = (
                np.asarray(trace["mem_use_amount"]) / 1e9
            )  # 1000M
            _data[trace_idx, service_idx, 4] = (
                np.asarray(trace["file_write_rate"]) / 1e8
            )
            ...
            _data[trace_idx, service_idx, 7] = (
                np.asarray(trace["net_receive_rate"]) / 1e8
            )
            _data[trace_idx, service_idx, 8] = list(
                map(lambda x: x // 100 if x != 0 else 9, (trace["http_status"]))
            )
        else:
            _data[trace_idx, service_idx, 0] = np.asarray(trace["latency"])
            _data[trace_idx, service_idx, 1] = list(
                map(lambda x: int(x) // 100 if x != 0 else 9, (trace["http_status"]))
            )
...
def trace_encoding(
    input_data: pd.DataFrame, cluster_id: str, drop_service=0, drop_fault_type=0
):
    exist_history_data = read_pkl_file(cluster_id + "encoded_history_trace.pkl")
    if exist_history_data is not None:
        ...
        write_pkl_file(cluster_id + "encoded_history_trace.pkl", res)
        return res

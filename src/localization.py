...
from rca_config import EXP_NOISE_LIST
...
def rc_localization(input_data,  min_support_rate=0.1, quiet=False, k=100, enable_prfl=False):
    ...
    for noise in [0]:
        ...
        toc = time.time()
        print(f'{noise=} speed={n_traces / (toc - tic):.2f}traces/second, time={toc - tic:.2f}s')

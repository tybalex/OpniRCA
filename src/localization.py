...
def rc_localization(
    ...
    output_data = {}
    tracerca = TraceRCA()
    n_traces = len(input_data["trace_id"].unique())
    # for noise in EXP_NOISE_LIST:
    for noise in [0]:
        injected_file = inject_noise(input_data, noise)
        tic = time.time()
        logger.info(f"RCA analysis for cluster : {cluster_id}")
        fp = tracerca(
            injected_file,
        ...
        toc = time.time()
        logger.debug(
            f"{noise=} speed={n_traces / (toc - tic):.2f}traces/second, time={toc - tic:.2f}s"
        )
...
class ItemsetHandler:
    def __init__(self, data: pd.DataFrame, **kwargs):
        ...
        self.items = reduce(
            lambda a, b: a | b, map(lambda x: x[0], self.itemsets)
        ...
        self._traces_containing_item = {
            key: frozenset(value) for key, value in item2trace_id_sets.items()
        }
        self._abnormal_traces_containing_item = {
            key: frozenset(value) for key, value in item2abnormal_trace_id_sets.items()
        }

        self.p_b = len(list(filter(lambda x: bool(x[1]), self.itemsets))) / len(
            self.itemsets
        )
    ...
    def _frequent_pattern_on_abnormal(self, **kwargs):
        ...
        abnormal_itemsets = list(
            map(lambda x: x[0], filter(lambda x: bool(x[1]), self.itemsets))
        )
        ...
        return {k: v for k, v in zip(frequent_itemsets, p_a_given_b)}, frequent_itemsets
    ...
    def abnormal_traces_containing_pattern(self, pattern: FrozenSet[str]):
        return reduce(
            lambda a, b: a | b, map(self.abnormal_traces_containing_item, pattern)
        ) & self.traces_containing_pattern(pattern)

    @staticmethod
    def gen_itemsets(data: pd.DataFrame, **kwargs):
        ...
        node_sets = list(
            ...
            for a, b in zip(
                trace_group[target_column].apply(set),
                trace_group[source_column].apply(set),
            )
        )
...
class PRFL:
    def __init__(
        self,
        data: pd.DataFrame,
        itemset_handler: ItemsetHandler,
        w=0.002,
        phi=0.5,
        d=0.6,
    ):
        ...
        A_oo = np.zeros(
            (len(involved_vertices), len(involved_vertices)), dtype=np.float
        )
        ...
        for trace_id in data.trace_id.unique():
            kind = frozenset(
                set(
                    trace_id_indexed_data.loc[[trace_id], ["source", "target"]]
                    .apply(tuple, axis=1)
                    .unique()
                )
            )
        ...
        for (a, b) in (
            data.loc[data.trace_id.isin(abnormal_traces), ["source", "target"]]
            .apply(tuple, axis=1)
            .unique()
        ):
        ...
        A = np.block(
            [
                [A_oo * w, A_ot],
                [A_to, np.zeros((len(abnormal_traces), len(abnormal_traces)))],
            ]
        )
        ...
        normal_kind = np.array(
            [
                1.0 / (len(kind2normal_traces[trace2kind[trace_id]]) + 1e-4)
                for trace_id in abnormal_traces
            ]
        )
        u_t_normal = normal_kind / np.sum(normal_kind)
        abnormal_kind = np.array(
            [
                1.0 / len(kind2abnormal_traces[trace2kind[trace_id]])
                for trace_id in abnormal_traces
            ]
        )
        ...
        u_t_abnormal = phi * n_operations / np.sum(n_operations) + (
            1 - phi
        ) * abnormal_kind / np.sum(abnormal_kind)

        v0 = np.block(
            [
                np.ones_like(u_o) / len(involved_vertices),
                np.ones_like(u_t_normal) / len(abnormal_traces),
            ]
        )

        self._pi_normal = self.solve_PPR(A, np.block([u_o, u_t_normal]), d=d, v0=v0)[
            : len(involved_vertices)
        ]
        self._pi_abnormal = self.solve_PPR(
            A, np.block([u_o, u_t_abnormal]), d=d, v0=v0
        )[: len(involved_vertices)]
        ...
        logger.debug(
            f"A has shape {np.shape(A)}, and costs {A.nbytes / 1024 / 1024:.2f} MBytes"
        )
    ...
    def __call__(
        ...
        if not enable_PRFL:
            ...
            def jaccard_similarity(_itemset):
                ...
                return 1 / (
                    1 / np.maximum(p_a_given_b, 1e-4)
                    + 1 / np.maximum(p_b_given_a, 1e-4)
                    - 1
                )
            ...
            def jaccard_similarity(_itemset):
                normal_score = reduce(
                    lambda x, y: x * y, [prfl.normal_score(_) for _ in _itemset], 1
                )
                abnormal_score = reduce(
                    lambda x, y: x * y, [prfl.abnormal_score(_) for _ in _itemset], 1
                )
                ...
                # ep = p * normal_score
                # np = p - ep
                ef = abnormal_score * len(
                    itemset_handler.abnormal_traces_containing_pattern(_itemset)
                )
                nf = abnormal_score * f - ef
                ep = normal_score * (
                    len(itemset_handler.traces_containing_pattern(_itemset))
                    - len(itemset_handler.abnormal_traces_containing_pattern(_itemset))
                )
        ...
        abnormal_data_indexed = input_file.set_index(
            [source_column, target_column, predict_column]
        ).xs(True, level=predict_column)
        abnormal_target_trace_ids = (
            abnormal_data_indexed.groupby(target_column)[trace_id_column]
            .apply(set)
            .to_dict()
        )
        abnormal_source_trace_ids = (
            abnormal_data_indexed.groupby(source_column)[trace_id_column]
            .apply(set)
            .to_dict()
        )
        ...
        def in_out_diff(_item, _pattern, return_="diff") -> float:
            if len(_pattern) <= 0:
                return 0.0
            _pattern_trace_ids = (
                itemset_handler.traces_containing_pattern(_pattern) & trace_id_sets
            )
            if len(_pattern_trace_ids) <= 0:
                return 0.0
            _target = len(
                abnormal_target_trace_ids.get(_item, frozenset()) & _pattern_trace_ids
            )
            _source = len(
                abnormal_source_trace_ids.get(_item, frozenset()) & _pattern_trace_ids
            )
            if return_ == "diff":
        ...
        pattern_scores = {
            pattern: jaccard_similarity(pattern)
            for pattern in itemset_handler.frequent_patterns
        }
        pattern_score_threshold = sorted(
            list(set(pattern_scores.values())), reverse=True
        )[:k][-1]
        patterns = list(
            filter(
        ...
        patterns = sorted(patterns, key=lambda x: pattern_scores[x], reverse=True)[
            :1000
        ]
        item_ret = defaultdict(
            lambda: {
                "score": -1,
                "pattern": frozenset(),
                "pattern_score": 0,
        ...
        ret = sorted(
            ...
            key=lambda _item: (
                item_ret[_item]["score"],
                -len(item_ret[_item]["pattern"]),
            ),
            reverse=True,
        )
        if not quiet:

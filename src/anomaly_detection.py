...
def train_history_model(trace_history_data, invo_history_data):
    ...
    invo_history = invo_history_data.set_index(keys=['source', 'target'], drop=False).sort_index()
...
def _anomaly_detection_3sigma_without_useful_features(df, result_column, cache, threshold):
    indices = np.unique(df.index.values)
    useful_feature = {key: FEATURE_NAMES for key in indices}
    return _anomaly_detection_3sigma(df, result_column, useful_feature, cache, threshold)
...
def span_anomaly_detection_main(input_data, useful_feature : dict, history_model, main_threshold=1):
    ...
    df = _anomaly_detection_3sigma_without_useful_features(df, 'NoSelection-predict', cache, threshold)
